...
file_handler = logging.FileHandler('train_log_hg.L23d_pmc')
...
def train(input_tfr_pool, val_tfr_pool, out_dir, log_dir, mean, sbatch, wd):
    ...
    with tf.Graph().as_default():
        ...
        images, gt_keys_hm = create_bb_pip(input_tfr_pool, 1000, sbatch, mean, shuffle=True)
        ...
        out_dim = 36
        pred_keys_hm = hg._graph_hourglass(input=images, nFeat=512, dropout_rate=0.2, outDim=out_dim, tiny=False, modif=False, is_training=True)
        ...
        total_loss = ut._bce_loss(logits=pred_keys_hm, gtMaps=gt_keys_hm, name='ce_loss', weighted=False)
        init_learning_rate = 2.5e-4 # to be deteremined
        # exp decay: 125000 / 2000 = 625decays,   0.992658^625 ~=0.01, 0.99^625 ~= 0.00187
        lr_hg = tf.train.exponential_decay(init_learning_rate, global_step=global_step, decay_rate=0.96, decay_steps=2000, staircase=True, name="learning_rate")
        ...
        if val_tfr_pool:
            ...
            for ix, val_tfr in enumerate(val_tfr_pool):
                ...
                # val_pred_key = sk_net.infer_key(val_images, key_dim, tp=False, reuse_=True)
                val_pred_keys_hm = hg._graph_hourglass(input=val_images, outDim=out_dim,is_training=False, tiny=False, modif=False, reuse=True)
