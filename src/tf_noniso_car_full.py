...
def noniso_warp(img, key2d, dim, method=cv2.INTER_CUBIC):
    ...
    # sanity check
    new_k2d *= dim
    new_k2d = np.transpose(new_k2d)

    new_k2d = np.array(new_k2d)
    pimg = np.array(pimg)
    #
    # for idx in range(18):
    #     cv2.circle(pimg, (int(new_k2d[0][idx]), int(new_k2d[1][idx])), 3, (0, 0, 255), -1)
    # for idx in range(18, 36):
    #     cv2.circle(pimg, (int(new_k2d[0][idx]), int(new_k2d[1][idx])), 3, (0, 255, 0), -1)
    # cv2.imshow('check', pimg)
    # cv2.waitKey(0)
...
def combine2datum(tuple_):
    ...
    # TL - single_car - 5 data augmentation methods
    if method == 0:
        left = bbx[0]
        bottom = bbx[1]
        right = bbx[2]
...
def main(argv):
    ...
    # root_path = '/home-1/cli53@jhu.edu/data/chi/syn_dataset'
    root_path = '/media/tliao4/671073B1329C337D/car_cp_from_chi'

    input_path = os.path.join(root_path, 'car_full', args.type)
    output_path = os.path.join(root_path, 'tfrecord/temp_test')
