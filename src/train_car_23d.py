...
def create_bb_pip(tfr_pool, nepoch, sbatch, mean, shuffle=True):
  ...
  for ix, tfr in enumerate(tfr_pool): 
    ...
    image, gt_2d, gt_3d = read_one_datum(fqueue, dim)

    if shuffle:
      data = tf.train.shuffle_batch([image, gt_2d, gt_3d], batch_size=cur_ebs, 
        num_threads=12, capacity=sbatch * 6, min_after_dequeue=cur_ebs * 3)
    else:
      data = tf.train.batch([image, gt_2d, gt_3d], batch_size=cur_ebs,
...
def train(input_tfr_pool, val_tfr_pool, out_dir, log_dir, mean, sbatch, wd):
  ...
  with tf.Graph().as_default():
    ...
    images, gt_2d, gt_3d = create_bb_pip(input_tfr_pool, 1000, sbatch, mean, shuffle=True)

    # inference model
    k2d_dim = gt_2d.get_shape().as_list()[1]
    k3d_dim = gt_3d.get_shape().as_list()[1]
    pred_key = sk_net.infer_23d(images, k2d_dim, k3d_dim, tp=True)
    
    # Calculate loss
    total_loss, data_loss = sk_net.L2_loss_23d(pred_key, [gt_2d, gt_3d], weight_decay=wd)
    ...
    if val_tfr_pool:
      ...
      for ix, val_tfr in enumerate(val_tfr_pool):
        ...
        val_images, val_gt_2d, val_gt_3d = create_bb_pip([val_tfr], 
            1000, sbatch, mean, shuffle=False)
        
        val_pred_key = sk_net.infer_23d(val_images, k2d_dim, k3d_dim, tp=False, reuse_=True)
        _, val_data_loss = sk_net.L2_loss_23d(val_pred_key, [val_gt_2d, val_gt_3d], None)
    ...
    with tf.Session(config=config) as sess:
      ...
      for i in xrange(total_iters):
        ...
        if i > 0 and i % log_freq == 0:
          ...
          key_loss, _ = sess.run([data_loss, train_op])
          ...
          sess.run([train_op])
          #img, _ = sess.run([images, train_op])
          #for ii in img:
          #  ii += 128
...
def main(FLAGS):
  ...
  train_files = ['syn_car_full_train_d64.tfrecord', 'syn_car_paddingmorecrop_train_d64.tfrecord', 'syn_car_multi_train_d64.tfrecord'] 
  val_files = ['syn_car_full_val_d64.tfrecord', 'syn_car_paddingmorecrop_val_d64.tfrecord', 'syn_car_multi_val_d64.tfrecord'] 
