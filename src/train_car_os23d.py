...
def create_bb_pip(tfr_pool, nepoch, sbatch, mean, shuffle=True):
  ...
  for ix, tfr in enumerate(tfr_pool): 
    ...
    image, gt_2d, gt_3d, gt_occ = read_one_datum(fqueue, dim)

    if shuffle:
      data = tf.train.shuffle_batch([image, gt_2d, gt_3d, gt_occ], batch_size=cur_ebs, 
        num_threads=12, capacity=sbatch * 6, min_after_dequeue=cur_ebs * 3)
    else:
      data = tf.train.batch([image, gt_2d, gt_3d, gt_occ], batch_size=cur_ebs,
...
def train(input_tfr_pool, val_tfr_pool, out_dir, log_dir, mean, sbatch, wd):
  ...
  with tf.Graph().as_default():
    ...
    images, gt_2d, gt_3d, gt_occ = create_bb_pip(input_tfr_pool, 1000, sbatch, mean, shuffle=True)
    ...
    if val_tfr_pool:
      ...
      for ix, val_tfr in enumerate(val_tfr_pool):
        ...
        val_images, val_gt_2d, val_gt_3d, _ = create_bb_pip([val_tfr], 
            1000, sbatch, mean, shuffle=False)
        
        val_pred_key = sk_net.infer_os(val_images, 36, tp=False, reuse_=True)
