...
def _graph_hourglass_modified_v1(input, nFeat=256, nLow=4, outDim=36, dropout_rate=0.2, nStack=4, modif=False, tiny=False, w_summary=False, is_training=False, reuse=False):
    """Create the stacke hourglass network
########################################################################################################################################################################################################
########################################################################################################################################################################################################
 1. prep: single_key_net... 3d intermediate supervision enabled with weight loaded(transfer learning)
 2. also increase nFeat to 512 instead of 256 as original version... now the input would be, instead of an image, will be preprocessed input
    e.g. (batch_size, 64, 64, 256)
...
SHG complete graph:
 ...
 ------------preprocesing_part---------------> HG_Module(stage:1) ---------------->dropout------>_conv_bn_relu------------------->conv*----------------->conv----------------->----------+----------> (@BP)
  (B,256,256,3)                (B,64,64,512)                       (B,64,64,512)                               (B,64,64,512)   |        (B,64,64,outDim)         (B,64,64,512)           |
                                                                                                                               |               ll_[0]*                                   |
...
ll_*: if not tiny, else just use ll.
 ...
 ------------(@BP)------------------------> HG_Module(stage:1) ---------------->dropout------>_conv_bn_relu------------------->conv*----------------->conv----------------->----------+----------> (@BP - recursion)
  (B,64,64,512)                (B,64,64,512)                       (B,64,64,512)                               (B,64,64,512)   |        (B,64,64,outDim)         (B,64,64,512)        |
                                                                                                                               |                                                      |
                                                                                                                               |               ll_[0]*                                |
